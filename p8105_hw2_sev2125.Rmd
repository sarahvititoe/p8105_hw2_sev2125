...
We have `r mta %>% group_by(line, station_name) %>% n_distinct()` unique stations. 
...
Only `r mta %>% select(line, station_name, ada) %>% group_by(line, station_name) %>% filter(ada == TRUE) %>% n_distinct()` of those stations are ADA compliant. :(
...
The proportion of all station etrances without a vending machine allow entrances is `r mta %>% filter(vending == "NO") %>% select(line, station_name, entry) %>% distinct() %>% summarise(proportion = mean(entry))`. 
...
There are `r mta_tidy %>% select(line, station_name, route_name, ada) %>% filter(route_name == "A") %>% n_distinct()` distinct stations that serve the A train.
...
There are `r mta_tidy %>% select(line, station_name, route_name, ada) %>% filter(route_name == "A" & ada == TRUE) %>% n_distinct()` distinct A train stations that are ADA accessible. 
...
*Clean names, focus on overall health, drop irrelevant variables, structure data so response types are variables, and create a new variable showing the proportion of responses that were excellend or very good*
```{r}
...
There are `r brfss %>% select(locationabbr, met_area) %>% n_distinct()` unique locations in our dataset. 
...
`r ifelse(brfss %>% select(locationabbr) %>%  filter(locationabbr != "dc") %>% n_distinct() >= 50, "We have all", "We only have")` `r brfss %>% select(locationabbr) %>% filter(locationabbr != "dc") %>% n_distinct()` states represented, plus `r brfss %>% select(locationabbr) %>% distinct() %>% anti_join(states, by = c("locationabbr" = "abbr")) %>% toupper()`.

*What state is observed the most?*

`r brfss %>% select(locationabbr) %>% group_by(locationabbr) %>% tally() %>% top_n(1) %>% left_join(states, by = c("locationabbr" = "abbr")) %>% select(name)` is the most observed state, with `r brfss %>% select(locationabbr) %>% group_by(locationabbr) %>% tally() %>% top_n(1) %>% select(n)` selected metropolitan areas in the study. 
